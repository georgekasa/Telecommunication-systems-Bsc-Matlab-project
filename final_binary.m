function [ P_binary] = final_binary( snr_db,diaspora,n )
if nargin == 0
	diaspora = 10^(-6);
	n = 10000;
	snr_db = 5;
...
x_n_binary = zeros(1,n/2); %
counter = 1;

for i = 1 :2: length(source)
	if source(i) == 0 && source(i+1)== 0
		x_n_binary(counter) = simvola(1);
	elseif source(i) ==0 && source(i+1) ==1
		x_n_binary (counter) = simvola(2);
	elseif source(i) ==1 && source(i+1) ==0
		x_n_binary (counter) = simvola(3);
	else`
		x_n_binary (counter) = simvola(4);
	end
	counter = counter+1;
end
%
...
counter =1 ;
receiver_bits_binary = zeros(size(source));
for j = 1 : length(receiver_coding)
	if (d_n(j) == simvola(1))
		receiver_bits_binary(counter)=0;
		receiver_bits_binary(counter+1)=0;
	elseif (d_n(j) ==simvola(2))
		receiver_bits_binary(counter)=0;
		receiver_bits_binary(counter+1)=1;
	elseif (d_n(j) ==simvola(3))
		receiver_bits_binary(counter) = 1 ;
		receiver_bits_binary(counter+1) = 0;
	else
		receiver_bits_binary(counter) = 1 ;
		receiver_bits_binary(counter+1) = 1;
...
for k = 1 : length(receiver_bits_binary)
	if receiver_bits_binary(k) ~= source(k)
		counter = counter+1;
	end
end
